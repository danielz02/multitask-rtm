...
class PCGrad():
    ...
    def cos_similarity(self, objectives):
        grads, shapes, has_grads = self._pack_grad(objectives)
        pc_grad, num_task = copy.deepcopy(grads), len(grads)

        tasks = list(objectives.keys())
        result = np.zeros(len(pc_grad) * len(pc_grad)).reshape((len(pc_grad), len(pc_grad)))
        ...
        for i in range(len(pc_grad)):
            ...
            for j in range(len(grads)):
                ...
                # compute cosine similarity, if < 0, then conflicting
                result[i][j] = g_i_g_j
        return result, tasks
